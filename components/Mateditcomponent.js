...
function Mateditcomponent({ material, setShowModal }) {
  ...
  useEffect(() => {
    (async () => {
      const result = await fetch(`/api/matgroup/materials/${material["matgroupid"]}`);
  ...
  return (
    // <div className="absolute inset-0 w-[100vw] h-full flex  opacity-90 bg-gray-50 justify-center  z-50 align-middle">
      <div className="fixed inset-0 z-40 w-full h-full bg-stone-900 opacity-90">
       ...
       <div className="fixed inset-1/2 z-50 w-[90%] h-[90%] justify-center align-middle -translate-x-1/2 -translate-y-1/2 bg-white opacity-100 rounded-[20px] border-r-2 border-b-2 ">
        ...
        <button className="fixed top-5 right-5 text-3xl font-semibold text-amber-900 bg-white px-3 py-1 border-2 hover:text-white hover:bg-red-600" onClick={() => setShowModal(false)}>X</button>
