...
export default function VendorEvaluationListPage({ track = 'current-year' }) {
  ...
  const [sortConfig, setSortConfig] = useState({
    key: ctx.isPriorPo ? 'lastPoDate' : 'totalValue',
    direction: 'desc',
  });
  ...
  const fetchVendors = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(withTrackQuery('/api/vendors/annual-evaluation', ctx.track));
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      if (sortConfig.key === 'lastPoDate') {
        aVal = a.lastPoDate ? new Date(a.lastPoDate).getTime() : 0;
        bVal = b.lastPoDate ? new Date(b.lastPoDate).getTime() : 0;
      }
  ...
  const renderScoreSummary = (v) => {
    ...
    return (
      <div className="text-xs text-gray-600 mb-1.5 leading-relaxed">
        ...
        {v.fixedOverall != null && v.variableOverall != null && (
        ...
        <>
          ...
          {v.approvedAt && (
            <div className="text-xs text-gray-400 mt-1">
              {moment(v.approvedAt).format('DD MMM YYYY')}
            </div>
          )}
        </>
      );
    ...
    if (v.evaluated) {
      return (
        <>
          ...
          {v.evaluatedAt && (
            <div className="text-xs text-gray-400 mt-1">
              {moment(v.evaluatedAt).format('DD MMM YYYY')}
            </div>
          )}
        </>
      );
  ...
  const renderActions = (v) => {
    const printButton = (
      <button
        type="button"
        onClick={() => window.open(`${ctx.listPath}/${v.vendorcode}/print`, '_blank')}
      ...
      >
        <FiPrinter className="mr-1" /> {v.approved ? 'Print PDF' : 'Draft Print'}
      </button>
    );
    ...
    if (v.evaluated) {
      const missing = v.missingSupplementary || [];
      const supplementaryComplete = Boolean(v.supplementaryComplete);
      const pendingLabel = missing.length
        ? `Pending: ${missing.join(' + ')}`
        : 'Pending additional params';
      ...
      return (
        <div className="flex flex-wrap items-center gap-2">
          {isSch && (
            <button
              type="button"
              onClick={() => router.push(`${ctx.listPath}/${v.vendorcode}?editScores=1`)}
          ...
          {!supplementaryComplete && (
            <button
              type="button"
              onClick={() =>
                router.push(
                  `${ctx.listPath}/${v.vendorcode}?section=additional-evaluation#additional-evaluation`
                )
              }
              title={`${pendingLabel}. Click to complete.`}
              className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-amber-900 bg-amber-100 border border-amber-300 rounded-md hover:bg-amber-200"
            >
          ...
          <button
            type="button"
            onClick={() =>
              setApprovalTarget({ vendorcode: v.vendorcode, vendorname: v.vendorname })
            }
            disabled={!supplementaryComplete}
            title={
              supplementaryComplete
                ? 'Approve evaluation'
                : `Complete ${missing.join(' and ') || 'additional evaluation parameters'} before approval`
            }
    ...
    return (
      <button
        type="button"
        onClick={() => router.push(`${ctx.listPath}/${v.vendorcode}`)}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="container mx-auto px-4 py-8 flex-1">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">{ctx.title}</h1>
          <p className="text-sm text-gray-600 mt-1">
            {ctx.isPriorPo ? (
              <>
                Active vendors with at least one PO who are <strong>not</strong> on Vendor
                Evaluation — Current Year (qualifying POs in {ctx.previousCalendarYear}). The most
                recent 2 purchase orders are evaluated. Fresh evaluation year:{' '}
                <strong>{ctx.evaluationYear}</strong>.
              </>
            ) : (
              <>
                Evaluating vendors with POs issued in <strong>{ctx.evaluationYear}</strong> (total
                PO value &gt; 10,000 SAR). This year updates automatically each January.
              </>
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
            <p className="text-sm text-gray-600">
              {ctx.isPriorPo ? 'Fresh Evaluation Year' : 'Evaluation Year'}
            </p>
            <p className="text-2xl font-bold text-blue-800">{ctx.evaluationYear}</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
            ...
            <p className="text-2xl font-bold text-green-800">
              {evaluatedCount} / {vendors.length}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-emerald-500">
            <p className="text-sm text-gray-600">Approved</p>
            <p className="text-2xl font-bold text-emerald-800">
              {approvedCount} / {vendors.length}
            </p>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    >
                      {ctx.isPriorPo ? 'Lifetime PO Value (SAR)' : 'Total PO Value (SAR)'}{' '}
                ...
                <tbody className="divide-y divide-gray-200">
                  {filtered.map((v) => (
                    <tr key={v.vendorcode} className="hover:bg-gray-50">
                      ...
                      {ctx.isPriorPo && (
                        <td className="px-6 py-4 text-sm text-gray-600">
                          {v.lastPoDate ? moment(v.lastPoDate).format('DD MMM YYYY') : '—'}
                        </td>
                      )}
