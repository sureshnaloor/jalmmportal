...
import VendorFeedbackRatingCard from '../VendorFeedbackRatingCard';
import POEvaluationCard from './POEvaluationCard';
import VendorEvaluationGroupSection from './VendorEvaluationGroupSection';
import SupplementaryEvaluationSections from './SupplementaryEvaluationSections';
...
export default function VendorEvaluationDetailPage({ track = 'current-year' }) {
  const router = useRouter();
  const { vendorcode, editScores: editScoresQuery, section } = router.query;
  ...
  useEffect(() => {
    if (loading || !vendorcode) return;
    const shouldScroll =
      section === 'additional-evaluation' ||
      (typeof window !== 'undefined' && window.location.hash === '#additional-evaluation');
    if (!shouldScroll) return;
    const timer = setTimeout(() => {
      document.getElementById('additional-evaluation')?.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    }, 250);
    return () => clearTimeout(timer);
  }, [loading, vendorcode, section]);
  ...
  const isEvaluated = Boolean(
    data?.evaluation && isEvaluationComplete(data.evaluation, requiredPoNumbers)
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const endpoint = editScoresMode
        ? withTrackQuery(`/api/vendors/annual-evaluation/${vendorcode}/edit-scores`, ctx.track)
        : withTrackQuery(`/api/vendors/annual-evaluation/${vendorcode}`, ctx.track);
      ...
      <main className="container mx-auto px-4 py-8 flex-1 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
            <p className="text-xs text-gray-500 uppercase">
              {ctx.isPriorPo ? 'Lifetime PO Value' : `Total ${ctx.evaluationYear} PO Value`}
            </p>
            <p className="text-xl font-bold text-blue-900">
              {data.totalPoValue?.toLocaleString()} SAR
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-indigo-500">
            <p className="text-xs text-gray-500 uppercase">
              {ctx.isPriorPo ? 'Lifetime PO Count' : 'PO Count'}
            </p>
            <p className="text-xl font-bold text-indigo-900">{data.poCount}</p>
            {ctx.isPriorPo && data.lastPoDate && (
              <p className="text-xs text-gray-500 mt-1">
                Last PO: {new Date(data.lastPoDate).toLocaleDateString()}
              </p>
            )}
          </div>
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-amber-500">
            ...
            <p className="text-xl font-bold text-amber-900">
              {fixedOverall != null ? `${fixedOverall} / 5` : '—'}
            </p>
          </div>
        </div>
        ...
        <section>
          <h2 className="text-lg font-semibold text-gray-800 mb-2">
            {ctx.isPriorPo
              ? 'Most Recent 2 POs — Variable Parameters'
              : 'Top 2 POs by Value — Variable Parameters'}
          </h2>
          <p className="text-sm text-gray-600 mb-6 italic">
            {ctx.isPriorPo
              ? `No qualifying PO in ${ctx.previousCalendarYear}. The most recent purchase orders are evaluated; overall marks are weighted based on PO value.`
          ...
          <div className="space-y-8">
            {data.topPOs?.map((po, idx) => (
              <POEvaluationCard
                ...
                rank={idx + 1}
                rankLabel={
                  ctx.isPriorPo ? `Most recent PO #${idx + 1}` : `Top PO #${idx + 1}`
                }
        ...
        <SupplementaryEvaluationSections
          ...
          highlight={section === 'additional-evaluation'}
          track={ctx.track}
        />
      </main>
      <FooterComponent />
