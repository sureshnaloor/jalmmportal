...
export async function ensureAnnualEvaluationShell(
  db,
  vendorcode,
  evaluationYear,
  rankedBy,
  ...
  const storageKey = storageKeyArg || getAnnualEvaluationStorageKey(evaluationYear);
...
export async function saveSupplementaryEvaluationSection({
  ...
  if (!existingEval) {
    await ensureAnnualEvaluationShell(db, code, evaluationYear, rankedBy, storageKey);
  }
...
export async function getSupplementaryEvaluationSection(
  db,
  vendorcode,
  fieldName,
  evaluationYear,
