...
const pdfStyles = StyleSheet.create({
  ...
  title: { fontSize: 18, fontWeight: 'bold', marginBottom: 6, color: '#1e3a8a' },
  subtitle: { fontSize: 10, color: '#475569', marginBottom: 10, lineHeight: 1.35 },
  poHeader: {
    fontSize: 11,
    fontWeight: 'bold',
    color: '#1e40af',
...
function formatLineMaterialLabel(line) {
  ...
  return desc || code || 'N/A';
}

function renderPoLineNodes(line, poNumber) {
  const key = `${poNumber}-line-${line.lineItem}`;
  const uom = (line.uom || '').trim();
  const qtyLabel = uom ? `${line.orderedQty} ${uom}` : String(line.orderedQty);

  return [
    <Text key={`${key}-sum`} style={pdfStyles.lineSummary}>
      {`Line ${line.lineItem} | PO Qty: ${qtyLabel} | GR Qty: ${line.grQty} | Pending: ${line.pendingValueSar.toLocaleString()} SAR`}
    </Text>,
    <Text key={`${key}-mat`} style={pdfStyles.lineMaterial}>
      {`Material/Service: ${formatLineMaterialLabel(line)}`}
    </Text>,
  ];
}
