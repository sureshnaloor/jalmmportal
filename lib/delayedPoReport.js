...
export async function buildDelayedPOReport(db, onProgress, options = {}) {
  ...
  for (const line of filteredLines) {
    ...
    if (openOnly && pendingValueSar <= 0) continue;

    const pendingQty = Math.max(
      0,
      parseDecimal(line['pending-qty']) || Math.max(0, orderedQty - deliveredQty)
    );
    ...
    reportLines.push({
        ...
        pendingValueSar,
        poStatus: pendingValueSar > 0 ? 'Open' : 'Closed',
  ...
  return (
    <Document>
      <Page size="A4" orientation="portrait" style={pdfStyles.page} wrap>
        ...
        {pos.length === 0 ? (
          <Text>No open PO lines found.</Text>
        ) : (
