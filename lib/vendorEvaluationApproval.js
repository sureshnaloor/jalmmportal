...
/**
 * Comma-separated approval emails from env.
 * Prefer NEXT_PUBLIC_SUPPLY_CHAIN_HEAD_EMAILS so the UI and API share the same list.
 * Server-only SUPPLY_CHAIN_HEAD_EMAILS is also accepted as a fallback.
 */
export function getSupplyChainHeadEmails() {
  const raw =
    process.env.NEXT_PUBLIC_SUPPLY_CHAIN_HEAD_EMAILS ||
    process.env.SUPPLY_CHAIN_HEAD_EMAILS ||
    '';
  return String(raw)
    .split(',')
...
export function isSupplyChainHead(email) {
  ...
  const allowed = getSupplyChainHeadEmails();
...
export function isSupplementaryEvaluationComplete(evaluation) {
  return Boolean(
    evaluation?.paymentTerms?.selectionId &&
      evaluation?.isoCertification?.selectionId
  );
}

export function getMissingSupplementarySections(evaluation) {
  const missing = [];
  if (!evaluation?.paymentTerms?.selectionId) missing.push('Payment Terms');
  if (!evaluation?.isoCertification?.selectionId) missing.push('ISO Certification');
  return missing;
}

export function canApproveEvaluation(evaluation, requiredPoNumbers = []) {
  return (
    isEvaluationComplete(evaluation, requiredPoNumbers) &&
    isSupplementaryEvaluationComplete(evaluation)
  );
}
...
export function buildEvaluationSummary(data) {
  ...
  return {
    ...
    track: data?.track || 'current-year',
    previousCalendarYear: data?.previousCalendarYear || null,
    ...
    supplementaryComplete: isSupplementaryEvaluationComplete(evaluation),
    missingSupplementary: getMissingSupplementarySections(evaluation),
  };
}
