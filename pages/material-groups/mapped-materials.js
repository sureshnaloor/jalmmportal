...
export default function MappedMaterialsPage() {
  ...
  const fetchMappedMaterials = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await fetch(`/api/materialsubgroupmap/mapped?${params.toString()}`);
      if (!response.ok) {
        throw new Error('Failed to fetch mapped materials');
      }
      ...
      console.error('Error fetching mapped materials:', err);
      setError('Failed to load mapped materials');
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const toggleSelectAll = () => {
    const pageCodes = items.map((item) => item.materialCode);
  ...
  const requestUpdate = () => {
    ...
    if (!selectedSubgroup) {
      setError('Please select a new group-subgroup combination');
      return;
    }
    setError(null);
  ...
  const executeUpdate = async () => {
    try {
      ...
      const response = await fetch('/api/materialsubgroupmap/update', {
        method: 'POST',
      ...
      setSuccess(
        `${data.updatedCount} mapping(s) updated to "${data.subgroup.groupName} - ${data.subgroup.name}".`
      );
      setSelectedCodes(new Set());
  ...
  const executeRemove = async () => {
    try {
      ...
      const response = await fetch('/api/materialsubgroupmap/remove', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          materialCodes: Array.from(selectedCodes),
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || 'Failed to remove mappings');
      }
      ...
      console.error('Error removing mappings:', err);
  ...
  const allOnPageSelected =
    items.length > 0 && items.every((item) => selectedCodes.has(item.materialCode));
  ...
  const confirmModalProps =
    confirmAction === 'update'
      ? {
          ...
          message: `Reassign ${selectedCodes.size} material${selectedCodes.size === 1 ? '' : 's'} to "${selectedSubgroup?.label || ''}"? Please verify the subgroup is correct.`,
      ...
      : confirmAction === 'remove'
        ? {
            ...
            message: `Remove subgroup mapping for ${selectedCodes.size} selected material${selectedCodes.size === 1 ? '' : 's'}? They will return to the unmapped list.`,
    ...
    <>
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <h1 className={styles.headerText}>Mapped Materials</h1>
        ...
        <div className={styles.content}>
          <div className={styles.groupsSection}>
            <h2>Mapped Materials</h2>
            <p className={styles.sectionHint}>
              Review and edit existing material-to-subgroup mappings. Select materials to reassign or remove mappings.
            </p>

            <div className={styles.searchSection}>
              <label className={styles.searchLabel}>Search materials (code, description, or subgroup)</label>
              <input
                type="text"
                className={styles.searchInput}
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="e.g. cable or civil*asphalt"
              />
            ...
            <div className={styles.paginationBar}>
              <span>
                {totalCount.toLocaleString()} mapped material{totalCount === 1 ? '' : 's'}
            ...
            <div className={styles.selectionBar}>
              <button
                type="button"
                className={styles.paginationButton}
                disabled={loading || items.length === 0 || allOnPageSelected}
            ...
            {loading ? (
              <div className={styles.loading}>Loading mapped materials...</div>
            ) : items.length === 0 ? (
              <p className={styles.emptyMessage}>
                {debouncedSearch
                  ? 'No mapped materials match your search.'
                  : 'No materials have been mapped yet.'}
              </p>
            ) : (
              <table className={styles.table}>
                ...
                <tbody>
                  {items.map((item) => {
                    const isSelected = selectedCodes.has(item.materialCode);
                    return (
                      <tr
                        key={item.mappingId}
                      ...
                      >
                        <td className={styles.checkboxCol}>
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => toggleMaterial(item.materialCode)}
                            onClick={(e) => e.stopPropagation()}
                          />
                        </td>
                        <td className={styles.groupName}>{item.materialCode}</td>
                        <td className={styles.groupDescription}>{item.materialDescription}</td>
                        <td className={styles.groupType}>{item.materialGroup || '—'}</td>
          ...
          <div className={styles.subgroupsSection}>
            <h2>Edit Mapping</h2>
            <p className={styles.sectionHint}>
              Choose a new subgroup to reassign selected materials, or remove the mapping entirely.
            </p>

            <div className={styles.mappingPanel}>
              <div className={styles.formField}>
                <label>New Group – Subgroup</label>
                <Select
                  options={subgroupOptions}
                  value={selectedSubgroup}
                  onChange={setSelectedSubgroup}
                  placeholder="Search and select new group-subgroup..."
                  isClearable
                  isSearchable
              ...
              {selectedSubgroup && (
                <div className={styles.selectedTarget}>
                  <p>
                    <strong>New target:</strong> {selectedSubgroup.label}
                  </p>
                </div>
              )}
              ...
              <div className={styles.formActions}>
                <button
                  ...
                  disabled={saving || selectedCodes.size === 0 || !selectedSubgroup}
                >
