...
function GroupVendorMappingPrintPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch('/api/materialgroups/vendor-mapping-report?format=pdf');
        if (!res.ok) throw new Error('Failed to generate vendor mapping PDF');
      ...
      <main className="flex-1 p-4" ref={iframePrintRef}>
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="mt-4 text-sm text-gray-600">Building the consolidated vendor mapping PDF…</p>
            <p className="mt-1 text-xs text-gray-500">This can take a minute for all groups and mapped vendors.</p>
          </div>
        ) : error ? (
...
export default GroupVendorMappingPrintPage;
