...
export default async function handler(req, res) {
  ...
  try {
    ...
    const existingMaterials = await db
      ...
      .find({ 'material-code': { $in: uniqueCodes } })
    ...
    if (codesToMap.length === 0) {
      return res.status(400).json({ error: 'None of the provided material codes exist' });
    }

    const existingMappings = await db
      .collection('materialsubgroupmap')
      .find({
        materialCode: { $in: codesToMap },
      })
    ...
    if (newCodes.length === 0) {
      return res.status(400).json({ error: 'All selected materials are already mapped' });
    }
    ...
    const result = await db.collection('materialsubgroupmap').insertMany(mappings);

    return res.status(201).json({
      message: `${newCodes.length} material(s) mapped successfully`,
    ...
    console.error('Error mapping materials to subgroup:', error);
    return res.status(500).json({ error: 'Failed to map materials' });
  }
}
