...
export default async function handler(req, res) {
  ...
  const collection = db.collection('pologs');
  ...
  if (req.method === 'POST') {
    const { project, title, priority, poNumber, requestInfo, createdBy, createdDate, status, lastUpdated } = req.body;
    if (!project || !title || !priority || !poNumber || !requestInfo) {
