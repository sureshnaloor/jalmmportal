...
export default async function handler(req, res) {
  ...
  const collection = db.collection('postdeliverylogs');
  ...
  if (req.method === 'POST') {
    const { project, title, priority, poReference, inspectionRef, requestInfo, createdBy, createdDate, status, lastUpdated } = req.body;
    if (!project || !title || !priority || !poReference || !inspectionRef || !requestInfo) {
